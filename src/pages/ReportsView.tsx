...
export const ReportsView: React.FC<ReportsViewProps> = ({ wasteLots, onOpenReport }) => {
  const exportCSV = () => {
    const headers = ['Batch ID', 'Generator', 'Waste Type', 'Quantity Tonnes', 'Facility', 'Pathway', 'Net CO2e', 'Status'];
    const rows = wasteLots.map((l) => [
      l.id,
      `"${l.generatorName}"`,
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={exportCSV}
            className="flex items-center gap-1.5 bg-surface hover:bg-surface-muted text-carbon-primary border border-border text-xs font-semibold px-3.5 py-2 rounded-btn transition"
          >
      ...
      <div className="space-y-4">
        {wasteLots.map((lot) => {
          ...
          return (
            <div key={lot.id} className="bg-surface border border-border rounded-card p-5 shadow-subtle flex flex-wrap items-center justify-between gap-4 hover:border-brand-primary/40 transition">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-btn bg-brand-soft text-brand-dark flex items-center justify-center flex-shrink-0">
